CREATE FUNCTION DM.GetEasterHolidays(@year INT) 
RETURNS TABLE
WITH SCHEMABINDING
AS 
RETURN 
(
  ...
  SELECT [Date], HolidayName = 'Easter Sunday' FROM x
    UNION ALL SELECT DATEADD(DAY, 40,[Date]), 'Ascension  Thursday' FROM x
	UNION ALL SELECT DATEADD(DAY, 50,[Date]), 'Pentecost Day' FROM x
