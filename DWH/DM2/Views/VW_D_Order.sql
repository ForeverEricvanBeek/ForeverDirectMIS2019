...
SELECT        
	...
	, CAST(STO.OLPN_Ship_Confirm_Date AS DATETIME2(7))	AS Order_Ship_Confirm_Date
	, CAST(STO.OLPN_First_Scan_Date AS DATETIME2(7))		AS Order_First_Scan_Date
	, CAST(ISNULL(STO.OLPN_First_Attempt_Date,STO.OLPN_Delivered_Date) AS DATETIME2(7)) AS Order_First_Attempt_Date
	, CAST(STO.OLPN_Delivered_Date AS DATETIME2(7))		AS Order_Delivered_Date
...
AND			OD.Eff_Date >= GETDATE() -2
AND			OD.End_Date >= GETDATE() -2
UNION ALL
...
SELECT        
		...
		, OD.DSG_SHIP_VIA				AS Order_Ship_Via_Code
		, FA.FACILITY_ALIAS_ID			AS Order_Facility_Code
		, OD.D_COUNTRY_CODE				AS Order_Country_Code
		, OD.EXT_PURCHASE_ORDER			AS Order_Pos_Number
		, CC.CARRIER_CODE				AS Order_Carrier_Code
		, OD.DWH_CUSTOMER_ID			AS Order_Customer_ID
