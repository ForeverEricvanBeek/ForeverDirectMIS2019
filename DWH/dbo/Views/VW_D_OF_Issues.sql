...
LEFT JOIN	(SELECT MAX(PT.CREATE_DATE_TIME) AS DATUM, PT.CNTR_NBR FROM ARC.PROD_TRKG_TRAN PT WHERE PT.MENU_OPTN_NAME IN ('PackCD','Pack Cubed Directed') GROUP BY PT.CNTR_NBR) X 
...
LEFT JOIN	(
				SELECT 
				   ...
				   , P2.LINE_ITEM_ID
				   , LH.DSP_LOCN
				   , RANK() OVER (PARTITION BY P2.CNTR_NBR, P2.LINE_ITEM_ID, LH.DSP_LOCN ORDER BY P2.CNTR_NBR, P2.LINE_ITEM_ID, LH.DSP_LOCN, dateadd(mi, datediff(mi, 0, P2.BEGIN_DATE), 0)) AS NUMBER
				 FROM		ARC.PROD_TRKG_TRAN P2 
...
SELECT DISTINCT
	...
	, CASE
         ...
         WHEN LD.INSTRTN_CODE_1 = 'L' THEN 'N not on correct location'
         WHEN LD.INSTRTN_CODE_1 = 'S' THEN 'N Status Issue'
     END							AS OF_Issues_Error
	, 'System'						AS OF_Issues_Error_Type
	, LD.INSTRTN_CODE_1				AS OF_Issues_Comments

FROM		ARC.LPN_DETAIL	AS LD
INNER JOIN	ARC.VW_T_LPN			AS LP
...
LEFT JOIN			(SELECT MAX(PT.CREATE_DATE_TIME) AS DATUM, PT.CNTR_NBR FROM ARC.PROD_TRKG_TRAN PT WHERE PT.MENU_OPTN_NAME = 'RF Anchor oLPN Pltz' GROUP BY PT.CNTR_NBR) X 
ON					X.CNTR_NBR = LP.TC_LPN_ID 
INNER JOIN			ARC.ORDERS AS OD
