...
WITH ORD_CTE (Customer_ID,Customer_Last_Order,Customer_First_Order_Date,Customer_Last_Order_Date,Customer_NumOf_Orders) AS (
SELECT
	X.Customer_ID
	, MAX(X.Customer_Order) AS Customer_Last_Order
	, MIN(X.CREATED_DTTM)	AS Customer_First_Order_Date
   , MAX(X.CREATED_DTTM)	AS Customer_Last_Order_Date 
...
SELECT 
	...
	, CAST(CC.Customer_Last_Order_Date AS DATETIME2(7))	AS Customer_Last_Order_Date
	, CAST(SUM(CC.Customer_NumOf_Orders) AS INT)				AS Customer_NumOf_Orders
FROM ORD_CTE AS CC
INNER JOIN MANH.ORDERS AS OD
ON CC.Customer_Last_Order = OD.TC_ORDER_ID
AND OD.ActInd = 'Y'
GROUP BY
	CAST(CC.Customer_ID	AS NVARCHAR(300))
	, CAST(OD.D_NAME AS NVARCHAR(100))
	, CAST(OD.D_ADDRESS_1 AS NVARCHAR(100))
	, CAST(OD.D_ADDRESS_2 AS NVARCHAR(100))
	, CAST(OD.D_CITY AS NVARCHAR(100))
	, CAST(OD.D_COUNTRY_CODE AS NVARCHAR(10))
	, CAST(OD.D_POSTAL_CODE	AS NVARCHAR(10))
	, CAST(OD.D_STATE_PROV AS NVARCHAR(10))
	, CAST(CC.Customer_First_Order_Date AS DATETIME2(7))
	, CAST(CC.Customer_Last_Order_Date AS DATETIME2(7))