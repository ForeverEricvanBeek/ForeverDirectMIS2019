...
ON				OD.TC_SHIPMENT_ID = SH.TC_SHIPMENT_ID
AND				SH.ActInd = 'Y'
JOIN			WEB.TransporterStatus AS TD 
ON				TD.TransporterStatus_ShipVia = OD.DSG_SHIP_VIA 
--AND				TD.Delivered = 1 
AND				TD.ActInd = 'Y' 
...
AND				FD.STATUS = TD.TransporterStatus_Status 
...
FROM(
		...
		ON				OD.TC_SHIPMENT_ID = SH.TC_SHIPMENT_ID
		AND				SH.ActInd = 'Y'
		LEFT JOIN		WEB.TransporterStatus AS TD 
		ON				TD.TransporterStatus_ShipVia = OD.DSG_SHIP_VIA 
--		AND				TD.Delivered = 1 
		AND				TD.ActInd = 'Y' 
