...
SELECT
	 --CONCAT(CASE WHEN Contract IS NULL THEN 'FD01' ELSE Contract END,SKU_Code) as SKU_Code
	 CONCAT(Contract,SKU_Code) as SKU_Code
	, Lot_Code
	, Status_Code
	, Status_Description
	, Received_Date
...
FROM (
		SELECT 
			...
			,BM.MOD_DATE_TIME
			, isnull(IP.INVENTORY_VALUE,0) AS Cost_Price
			...
			LEFT OUTER JOIN  IFS.INVENTORY_PART_IN_STOCK IPIS ON BM.BATCH_NBR = IPIS.LOT_BATCH_NO and IPIS.PART_NO=IC.ITEM_NAME and IPIS.ActInd='Y'
