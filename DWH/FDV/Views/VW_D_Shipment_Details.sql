...
CREATE VIEW [FDV].[VW_D_Shipment_Details]
AS
...
SELECT               
    ...
    ,   MAX(VV.PARCELL_COLLECT_DATE) AS [Shipment Collection Date]
    ,   MAX(PARCEL_DELIVERED_PICKUP_LOCATION) AS [Shipment Delivered at Pickup Date]  
    ,   MAX(PARCEL_RECEIVED_BY_CUSTOMER)  AS [Shipment Delivered at Customer Date] 
    ,   MAX(PARCEL_RETURNED_TO_FD) AS [Shipment Delivered at FD Date]
    ,	MAX(PARCEL_LAST_DELIVERY_DATE) AS [Shipment Last Delivery Date]
