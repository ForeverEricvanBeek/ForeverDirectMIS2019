...
SELECT			
	...
	, CASE WHEN SP.USE_SITE_SPECIFIC_DB = 'TRUE' THEN 'BK' ELSE 'OF' END as Order_Type
	, INVP.ACCOUNTING_GROUP			AS Accounting_Group_ID
	, CASE WHEN INVP.ACCOUNTING_GROUP =10 THEN 'Finished Goods'
	       WHEN INVP.ACCOUNTING_GROUP =20 THEN 'Literature L4L'
		   WHEN INVP.ACCOUNTING_GROUP =40 THEN 'Consumable(FD)'
		   WHEN INVP.ACCOUNTING_GROUP =50 THEN 'Raw Materials'
		   WHEN INVP.ACCOUNTING_GROUP =55 THEN 'Raw Materials WIP'
		   WHEN INVP.ACCOUNTING_GROUP =30 THEN 'Labels'
		   WHEN INVP.ACCOUNTING_GROUP =21 THEN 'AVA Literature'
		   WHEN INVP.ACCOUNTING_GROUP =22 THEN 'Global Marketing LT - non-EU'
		   WHEN INVP.ACCOUNTING_GROUP =23 THEN 'FD - LT + CTN'
		   WHEN INVP.ACCOUNTING_GROUP =45 THEN 'Product Cartons'
		   WHEN INVP.ACCOUNTING_GROUP =24 THEN 'Global Marketing LT - EU'
		   WHEN INVP.ACCOUNTING_GROUP =60 THEN 'Components AVE' END as Accounting_Group_Desc
...
ON				SP.PART_NO=INVP.PART_NO
AND				SP.ActInd='Y'

LEFT JOIN		IFS.COMMODITY_GROUP CG
ON				INVP.SECOND_COMMODITY = CG.SECOND_COMMODITY
AND				CG.ActInd = 'Y' 			
LEFT JOIN		IFS.PURCHASE_PART_SUPPLIER PPS
ON				PPS.PART_NO = INVP.PART_NO
and				PPS.CONTRACT=INVP.CONTRACT
AND				PPS.ActInd = 'Y'
AND				PPS.PRIMARY_VENDOR_DB = 'Y'
--AND				PPS.CONTRACT='FD01'
...
AND				UN.ActInd = 'Y'
WHERE			I.ActInd = 'Y'
--and				I.ITEM_NAME='039L'
UNION
SELECT     - 1, '-1', NULL,NULL,NULL,NULL,NULL,NULL, 'Unknown', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NUll, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL