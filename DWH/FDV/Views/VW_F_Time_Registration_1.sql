...
CREATE VIEW [FDV].[VW_F_Time_Registration_1]
AS
...
FROM (
	...
	INNER JOIN (
		...
		FROM ATIMO.timewize AS tw
		WHERE tw.ActInd = 'Y'
		GROUP BY tw.pers_key
			,tw.bkdate
		) AS T1 ON T1.track_id = tw.track_id
		AND T1.pers_key = tw.pers_key
		AND T1.bkdate = tw.bkdate
	WHERE (tw.ActInd = 'Y')
	...
	PIVOT(SUM(bkminutes) FOR boektype IN (
				[99]
				,[100]
				,[112]
				,[71]
